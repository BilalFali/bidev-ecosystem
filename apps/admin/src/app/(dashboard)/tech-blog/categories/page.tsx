...
import type { TechCategory } from "@/lib/types/techblog-database";
...
export default function TechCategoriesPage() {
  ...
  async function fetchCategories() {
    setLoading(true);
    const res = await fetch("/api/tech-blog/categories");
  ...
  async function handleDelete(id: string) {
    ...
    await fetch("/api/tech-blog/categories", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      <DashboardHeader
        title="Tech Blog Categories"
        description={`${categories.length} categor${categories.length !== 1 ? "ies" : "y"} — the fixed taxonomy from TAXONOMY.md`}
        actions={
