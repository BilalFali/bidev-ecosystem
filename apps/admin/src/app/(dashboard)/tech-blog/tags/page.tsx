...
import type { TechTag } from "@/lib/types/techblog-database";

export default function TechTagsPage() {
  ...
  async function fetchTags() {
    setLoading(true);
    const res = await fetch("/api/tech-blog/tags");
  ...
  async function handleDelete(id: string) {
    ...
    await fetch("/api/tech-blog/tags", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id }) });
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      <DashboardHeader
        title="Tech Blog Tags"
        description={`${tags.length} tag${tags.length !== 1 ? "s" : ""} — keep this list curated, per TAXONOMY.md §5`}
        actions={
