...
import { Plus, Search, Pencil, ExternalLink, AlertCircle, CheckCircle2 } from "lucide-react";
...
interface SearchParams { status?: string; category?: string; search?: string }

function isComplete(a: ArticleWithRelations): boolean {
  return Boolean(
    a.troubleshooting_category_id &&
    a.excerpt &&
...
export default async function TroubleshootingPage({ searchParams }: { searchParams: Promise<SearchParams> }) {
  ...
  const [{ data: articles }, { data: categories }] = await Promise.all([
    query,
    supabase.from("troubleshooting_categories").select("id,name,slug").order("sort_order"),
  ]);

  let list = (articles ?? []) as ArticleWithRelations[];
  if (params.category && params.category !== "all") {
    list = list.filter((a) => a.troubleshooting_category_slug === params.category);
  }
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      <DashboardHeader
        ...
        description={`${list.length} guide${list.length !== 1 ? "s" : ""}`}
        actions={
          <Link href="/articles/new?troubleshooting=1">
            <Button size="sm"><Plus className="w-3.5 h-3.5" />New Troubleshooting Guide</Button>
          </Link>
        }
      />
      ...
      <div className="flex flex-wrap items-center gap-3 mb-5">
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          <Link
            href="/troubleshooting?category=all"
          ...
          {(categories ?? []).map((c) => (
            <Link
              key={c.slug}
              href={`/troubleshooting?category=${c.slug}`}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
      ...
      <div className="bg-bg-elevated border border-border rounded-xl overflow-hidden">
        {list.length === 0 ? (
          <div className="py-16 text-center">
            <p className="text-sm text-ink-muted">No troubleshooting guides yet.</p>
            <Link href="/articles/new?troubleshooting=1" className="text-sm text-accent hover:underline mt-1 inline-block">
              Create your first troubleshooting guide →
            </Link>
          </div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="border-b border-border text-xs text-ink-muted uppercase tracking-wide">
                ...
                <th className="text-left px-4 py-3 font-medium hidden lg:table-cell">Completeness</th>
            ...
            <tbody className="divide-y divide-border">
              {list.map(article => {
                const complete = isComplete(article);
                return (
                  <tr key={article.id} className="hover:bg-bg-card/50 transition-colors group">
                    ...
                    <td className="px-4 py-3.5 hidden sm:table-cell">
                      <span className="text-xs text-ink-muted">
                        {article.troubleshooting_category_name ?? "—"}
                      </span>
                    </td>
