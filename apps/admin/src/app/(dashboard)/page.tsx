...
import { FileText, Image as ImageIcon, TrendingUp, Plus, ArrowRight, Newspaper } from "lucide-react";
...
async function getTechBlogStats() {
  try {
    ...
    const [
      ...
      tb.from("techblog_articles").select("*", { count: "exact", head: true }),
      tb.from("techblog_articles").select("*", { count: "exact", head: true }).eq("status", "published"),
      tb.from("techblog_articles").select("*", { count: "exact", head: true }).eq("status", "draft"),
      tb.from("techblog_articles_with_relations").select("*").order("updated_at", { ascending: false }).limit(6),
    ]);
    ...
    return {
      ...
      published:      published ?? 0,
      draft:          draft     ?? 0,
      recentArticles: (recentArticles ?? []) as TechArticleWithRelations[],
    };
  } catch {
    return { total: 0, published: 0, draft: 0, recentArticles: [] };
  }
}

export default async function OverviewPage() {
  const [stats, techStats] = await Promise.all([getStats(), getTechBlogStats()]);
  ...
  return (
    <div className="max-w-5xl mx-auto animate-fade-in">
      ...
      {/* Recent articles */}
      <div className="bg-bg-elevated border border-border rounded-xl overflow-hidden mb-8">
      ...
      {/* Tech Blog section — separate Supabase project, tech.bidev.dev */}
      <DashboardHeader
        title="Tech Blog"
        description="BiDev Tech (tech.bidev.dev) — a separate publication"
        actions={
          <Link href="/tech-blog/articles/new">
            <Button size="sm" variant="secondary"><Plus className="w-3.5 h-3.5" />New Tech Article</Button>
          </Link>
        }
        className="mt-2"
      />
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <StatsCard label="Total Articles" value={techStats.total}     icon={Newspaper}  />
        <StatsCard label="Published"      value={techStats.published} icon={TrendingUp} accent />
        <StatsCard label="Drafts"         value={techStats.draft}     icon={Newspaper}  />
      </div>

      <div className="bg-bg-elevated border border-border rounded-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <h2 className="text-sm font-semibold text-ink">Recent Tech Blog Articles</h2>
          <Link href="/tech-blog/articles" className="text-xs text-accent hover:text-accent-hover flex items-center gap-1 transition-colors">
            View all <ArrowRight className="w-3 h-3" />
          </Link>
        </div>

        {techStats.recentArticles.length === 0 ? (
          <div className="px-5 py-12 text-center">
            <Newspaper className="w-8 h-8 text-ink-faint mx-auto mb-3" />
            <p className="text-sm text-ink-muted">No tech blog articles yet.</p>
          ...
          <div className="divide-y divide-border">
            {techStats.recentArticles.map(article => (
              <div key={article.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-bg-card/50 transition-colors group">
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium text-ink truncate">{article.title}</p>
                  <p className="text-xs text-ink-muted mt-0.5">
