...
export async function PUT(req: NextRequest, { params }: Ctx) {
  ...
  const { data: article, error } = await tb
  ...
  await tb.from("techblog_article_tags").delete().eq("article_id", id);
  if (tag_ids?.length) {
    await tb.from("techblog_article_tags").insert(
...
export async function DELETE(_req: NextRequest, { params }: Ctx) {
  ...
  const tb = createTechBlogClient();
  const { id } = await params;
  const { error } = await tb.from("techblog_articles").delete().eq("id", id);
  if (error) return NextResponse.json({ error: error.message }, { status: 500 });
