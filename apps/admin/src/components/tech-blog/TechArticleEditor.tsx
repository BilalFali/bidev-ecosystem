...
import { TechEditorSidebar } from "./TechEditorSidebar";
import { Button } from "@/components/ui/Button";
import { Alert } from "@/components/ui/Alert";
import { Modal } from "@/components/ui/Modal";
import { slugify, estimateReadingTime } from "@/lib/utils";
import type {
  TechArticleFormData, TechArticleWithRelations, TechCategory, TechSection, TechTag,
} from "@/lib/types/techblog-database";

const EMPTY_FORM: TechArticleFormData = {
  title: "",
  slug: "",
  dek: "",
  content: "",
...
interface TechArticleEditorProps {
  article?: TechArticleWithRelations;
...
export function TechArticleEditor({ article, categories, sections, tags }: TechArticleEditorProps) {
  const router = useRouter();
  const isNew  = !article;

  const [form, setForm] = useState<TechArticleFormData>(() => ({
    ...EMPTY_FORM,
    ...(article ? {
      title:           article.title,
      slug:            article.slug,
  ...
  function setField<K extends keyof TechArticleFormData>(key: K, value: TechArticleFormData[K]) {
  ...
  async function save(mode: "manual" | "auto" | "publish" | "unpublish" = "manual") {
    ...
    try {
      const url    = isNew ? "/api/tech-blog/articles" : `/api/tech-blog/articles/${article!.id}`;
      ...
      if (isNew) {
        router.replace(`/tech-blog/articles/${saved.id}/edit`);
      } else {
  ...
  async function handleDelete() {
    try {
      await fetch(`/api/tech-blog/articles/${article!.id}`, { method: "DELETE" });
      router.push("/tech-blog/articles");
    } catch {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-6 h-14 border-b border-border bg-bg-secondary shrink-0">
        <Link href="/tech-blog/articles" className="flex items-center gap-1.5 text-sm text-ink-muted hover:text-ink transition-colors shrink-0">
